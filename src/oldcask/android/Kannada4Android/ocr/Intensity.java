...
public class Intensity {
	...
	public static Bitmap doBrighten(Bitmap img) {
		...
		for (int i = 0; i < w; i++) {
			for (int j = 0; j < h; j++) {
		        ...
		        if ( r == 0 && g == 0 && b == 0) {
		           img.setPixel(i, j, Color.rgb(k, k, k));
		        }
		        if ( r > 0 && r < i ) r = i;
		        if ( g > 0 && g < i ) g = i;
		        if ( b > 0 && b < i ) b = i;

		        img.setPixel(i, j, Color.rgb(Math.min((int)(r/FACTOR), 255),
	...
	public static Bitmap doDarken(Bitmap img) {
		...
		for (int i = 0; i < w; i++) {
			for (int j = 0; j < h; j++) {
				int rgb = img.getPixel(i, j);
				img.setPixel(i, j, Color.rgb(Math.max((int)(getRed(rgb) *FACTOR), 0), 
